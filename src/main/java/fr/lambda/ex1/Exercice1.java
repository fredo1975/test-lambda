...
public class Exercice1 {
    public static void main(String[] args){
        ...
        List<String> res = l.stream().filter(e -> e.getAge() > 20).map(Employee::getName).toList();
        ...
        int max = l.stream().max(Comparator.comparingInt(Employee::getAge)).get().getAge();
        System.out.println("Maximum age of employee: "+max);

        //Given a list of employees, sort all the employee on the basis of age
        List<Employee> sorted = l.stream().sorted(Comparator.comparingInt(Employee::getAge)).toList();
        System.out.println("Employees sorted by age: ");
        sorted.forEach(System.out::println);

        //Join the all employee names with “,”
        String joinedNames = l.stream().map(Employee::getName).reduce((a, b) -> a+","+b).get();
        ...
        OptionalDouble average = nums.stream().mapToInt(value -> value*value).filter(v->v>10000).average();
